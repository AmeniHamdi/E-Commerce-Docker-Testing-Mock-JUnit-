...
public class SpringFoxSwaggerConfig {
	...
	public Docket api() {
		return new Docket(DocumentationType.SWAGGER_2).apiInfo(apiEndPointsInfo())
				...
				.select().apis(RequestHandlerSelectors.basePackage("com.esprit.examen.controller"))
				.paths(PathSelectors.any()).build();
	}

	private ApiInfo apiEndPointsInfo() {
		return new ApiInfoBuilder().title("My STOCK PROJECT").description("Micro-Service Documentation")
				.version("1.0.0").build();
	}
